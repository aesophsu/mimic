...
def _run_one_target(
    ...
    usable = [f for f in ranked if f in df_train.columns and f in df_test.columns and f in df_eicu.columns]
    if not usable:
        raise ValueError(f"{target}: 无可用特征可用于精简外部验证")
    k_use = min(k, len(usable))
    feats = usable[:k_use]
    ...
    with open(os.path.join(target_dir, "external_validation_slim_config.json"), "w", encoding="utf-8") as f:
        json.dump({"target": target, "k": k_use, "features": feats, "threshold": threshold}, f, ensure_ascii=False, indent=2)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="精简版（k）XGBoost eICU 外部验证")
    parser.add_argument("--targets", nargs="+", default=OUTCOMES, help=f"结局列表，默认: {' '.join(OUTCOMES)}")
    parser.add_argument("--k-pof", type=int, default=DEFAULT_K["pof"], help="POF 的精简特征数")
    parser.add_argument("--k-mortality", type=int, default=DEFAULT_K["mortality"], help="Mortality 的精简特征数")
    parser.add_argument("--k-composite", type=int, default=DEFAULT_K["composite"], help="Composite 的精简特征数")
    ...
    k_map = {
        "pof": int(args.k_pof),
        "mortality": int(args.k_mortality),
        "composite": int(args.k_composite),
    }
    ...
    for target in [t.lower() for t in args.targets]:
        ...
        if target not in df_eicu.columns:
            ...
            continue
        try:
            rows.append(_run_one_target(target, k_map[target], df_train, df_test, df_eicu))
        except Exception as e:
